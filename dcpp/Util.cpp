...
string Util::getAwayMessage() {
	...
	if (SETTING(AWAY_TIMESTAMP)) {
		return msg.empty() ? msg : formatTime(msg + "[%x %X]", awayTime) + " <" + APPNAME + " " VERSIONSTRING ">";
	} else {
		return msg.empty() ? msg : formatTime(msg, awayTime) + " <" + APPNAME + " " VERSIONSTRING ">";
	}
}
...
void Util::setAway(bool b) {
	...
	SettingsManager::getInstance()->set(SettingsManager::LAST_AWAY, getAway());
}
